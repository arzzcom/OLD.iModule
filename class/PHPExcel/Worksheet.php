...
/** PHPExcel */
require_once PHPEXCEL_ROOT . 'PHPExcel.class.php';
...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	*/
	...
	public function copySheet($rowLine,$loopNum) {
		$loop = 1;

		while ($loop < $loopNum) {
			for ($i='A';$i<=$this->getHighestColumn();$i++) {
				for ($j=1;$j<=$rowLine;$j++) {
					$thisCell = $this->_cellCollection[$i.$j];
					if (!$thisCell) break;
					$copyCell = $this->_cellCollection[$i.($j+($loop*$rowLine))] = new PHPExcel_Cell($i,$j+($loop*$rowLine),$thisCell->getValue(),$thisCell->getDataType(),$this);
					$this->duplicateStyle($this->getStyle($i.$j),$i.($j+($loop*$rowLine)));
					$thisRow = $this->_rowDimensions[$j];
					$copyRow = $this->_rowDimensions[$j+($loop*$rowLine)] = new PHPExcel_Worksheet_RowDimension($j+($loop*$rowLine));
					$copyRow->setRowHeight($thisRow->getRowHeight());
					$copyRow->setVisible($thisRow->getVisible());
					$copyRow->setOutlineLevel($thisRow->getOutlineLevel());
					$copyRow->setCollapsed($thisRow->getCollapsed());
				}
			}
			$loop++;
		}
		...
		while ($loop < $loopNum) {
			foreach ($mergeCells as $cell=>$value) {
				if (preg_match('/([A-Z]+)([0-9]+):([A-Z]+)([0-9]+)/',$cell,$match) == true) {
					$mStartCol = $match[1];
					$mStartRow = $match[2];
					$mEndCol = $match[3];
					$mEndRow = $match[4];

					$this->_mergeCells[$mStartCol.($mStartRow+$rowLine*$loop).':'.$mEndCol.($mEndRow+$rowLine*$loop)] = $mStartCol.($mStartRow+$rowLine*$loop).':'.$mEndCol.($mEndRow+$rowLine*$loop);
				}
			}
