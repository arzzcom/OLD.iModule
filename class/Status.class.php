...
class Status {
	...
	function GetBotName($botname) {
		$botName = array('NaverBot'=>'네이버봇(NaberBot)','Daumoa'=>'다음봇(Daumoa)','Googlebot'=>'구글봇(Googlebot)','Yeti'=>'네이버봇(Yeti)','bingbot'=>'Bing봇(bingbot)','Mediapartners-Google'=>'구글(Mediapartners-Google)','nagios-plugins'=>'Nagios(nagios-plugins)','checkprivacy'=>'한국인터넷진흥원(KISA)','first'=>'한국인터넷진흥원(KISA-FIRST)','facebook'=>'페이스북');
		return isset($botName[$botname]) == true ? $botName[$botname] : '기타('.$botname.')';
	}
	...
	function SaveStatus() {
		...
		if ($isBot == false && Request('CheckVisitor','cookie') != $date && Request('CheckVisitor','session') != $date) {
			...
			if (isset($_SERVER['HTTP_REFERER']) == true && $_SERVER['HTTP_REFERER'] && preg_match('/'.$_SERVER['HTTP_HOST'].'/',$_SERVER['HTTP_REFERER']) == false) {
				...
				$this->mDB->DBinsert($this->table['referer'],$insert);
			}
		} else {
			$isVisitor = false;
		}
