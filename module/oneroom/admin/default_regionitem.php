		...
		menu.add({
			...
			handler:function(item) {
				...
				Ext.Ajax.request({
					...
					params:{"action":"item","do":"defaultregionitemmode","value":value,"idx":record.data.idx}
				});
			}
		});
			...
			new Ext.grid.GridPanel({
					...
					new Ext.Button({
						...
						handler:function(button) {
							if (button.pressed == false) {
								...
								Ext.getCmp("ListRegionItem").toggle(false);
								Ext.getCmp("ListDefault").toggle(false);
								
								button.toggle(true);
								store.getProxy().setExtraParam("is_default_regionitem","");
								store.loadPage(1);
							}
						},
					...
					new Ext.Button({
						...
						handler:function(button) {
							if (button.pressed == false) {
								...
								button.toggle(true);
								store.getProxy().setExtraParam("is_default_regionitem","TRUE");
								store.loadPage(1);
							}
						},
						listeners:{toggle:{fn:function(button,pressed) {
					...
					new Ext.Button({
						...
						handler:function(button) {
							if (button.pressed == false) {
								...
								button.toggle(true);
								store.getProxy().setExtraParam("is_default_regionitem","FALSE");
								store.loadPage(1);
							}
						},
						listeners:{toggle:{fn:function(button,pressed) {
							...
							items:[{
								...
								handler:function() {
									...
									Ext.Ajax.request({
										...
										params:{"action":"item","do":"defaultregionitemmode","value":"TRUE","idx":idxs.join(",")}
									});
								}
							},{
				...
				columns:[
					new Ext.grid.RowNumberer({
						...
						header:"지역추천여부",
						dataIndex:"is_default_regionitem",
						width:85,
						renderer:function(value) {
							if (value == "TRUE") return '<span style="color:red;">기본지역추천</span>';
							else return '일반매물';
						}
					},{
						header:"매물명",
