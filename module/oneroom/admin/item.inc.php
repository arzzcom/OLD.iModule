			...
			new Ext.Button({
				...
				handler:function() {
					...
					Ext.getCmp("ItemFormPanel").getForm().submit({
						url:"<?php echo $_ENV['dir']; ?>/module/oneroom/exec/Admin.do.php?action=item&do="+(idx ? "modify&idx="+idx : "add"),
		...
		listeners:{show:{fn:function() {
			...
			if (idx) {
				Ext.getCmp("ItemFormPanel").getForm().load({
					url:"<?php echo $_ENV['dir']; ?>/module/oneroom/exec/Admin.get.php?action=item&get=data&idx="+idx,
					submitEmptyText:false,
					waitTitle:"잠시만 기다려주십시오.",
					waitMsg:"데이터를 로딩중입니다.",
					success:function(form,action) {
