...
$idx = Request('idx');
if ($idx != null) {
	$data = $mDB->DBfetch($mOneroom->table['file'],array('repto','filename','filepath','filetype','filesize'),"where `idx`='$idx'");
	$mDB->DBupdate($mOneroom->table['file'],'',array('hit'=>'`hit`+1'),"where `idx`=$idx");

	if (file_exists($_ENV['userfilePath'].$mOneroom->userfile.$data['filepath']) == true) {
		...
		$fp = fopen($_ENV['userfilePath'].$mOneroom->userfile.$data['filepath'],'r');
		while(!feof($fp)) {
