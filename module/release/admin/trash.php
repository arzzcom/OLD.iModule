...
var ContentArea = function(viewport) {
	...
	var store = new Ext.data.JsonStore({
		proxy:{
			...
			extraParams:{action:"trash",key:"",keyword:"",category:"",rid:""}
		},
		remoteSort:true,
		sorters:[{property:"idx",direction:"DESC"}],
		autoLoad:true,
		pageSize:50,
		fields:["idx","rid","releasetitle","category","title","mno","name","nickname","width","newment",{name:"ment",type:"int"},{name:"trackback",type:"int"},{name:"hit",type:"int"},{name:"vote",type:"int"},{name:"avgvote",type:"float"},"reg_date","file","ip"]
	});
	...
	function ItemContextMenu(grid,record,row,index,e) {
		...
		menu.add({
			text:"게시물 복원",
			handler:function() {
				Ext.Msg.show({title:"확인",msg:"선택한 휴지통게시물을 복원하시겠습니까?",buttons:Ext.Msg.YESNO,icon:Ext.Msg.QUESTION,fn:function(button) {
					if (button == "yes") {
						Ext.Msg.wait("선택한 게시물을 복원하고 있습니다.","잠시만 기다려주십시오.");
						Ext.Ajax.request({
							url:"<?php echo $_ENV['dir']; ?>/module/release/exec/Admin.do.php",
							success:function(response) {
								var data = Ext.JSON.decode(response.responseText);
								if (data.success == true) {
									Ext.Msg.show({title:"안내",msg:"성공적으로 복원하였습니다.",buttons:Ext.Msg.OK,icon:Ext.Msg.INFO,fn:function() {
							...
							params:{"action":"trash","do":"recover","idx":record.data.idx}
						});
					}
				}});
			}
		...
		menu.add({
			text:"게시물 완전삭제",
			handler:function() {
				Ext.Msg.show({title:"확인",msg:"선택한 게시물을 정말 삭제하시겠습니까?<br />휴지통에서 삭제된 게시물은 다시 복구할 수 없습니다.",buttons:Ext.Msg.YESNO,icon:Ext.Msg.QUESTION,fn:function(button) {
					if (button == "yes") {
						...
						Ext.Ajax.request({
							...
							params:{"action":"trash","do":"delete","idx":record.data.idx}
						});
					}
				}});
			}
			...
			new Ext.grid.GridPanel({
				...
				tbar:[
						...
						menu:new Ext.menu.Menu({
							items:[{
								...
								handler:function() {
									...
									if (checked.length == 0) {
										Ext.Msg.show({title:"에러",msg:"복원할 게시물을 선택하여 주십시오.",buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
										return false;
									}
									...
									Ext.Msg.show({title:"확인",msg:"선택한 휴지통게시물을 복원하시겠습니까?",buttons:Ext.Msg.YESNO,icon:Ext.Msg.QUESTION,fn:function(button) {
										if (button == "yes") {
											...
											Ext.Ajax.request({
												...
												params:{"action":"trash","do":"recover","idx":idxs.join(",")}
											});
										}
									}});
								}
									...
									Ext.Msg.show({title:"확인",msg:"선택한 게시물을 정말 삭제하시겠습니까?<br />휴지통에서 삭제된 게시물은 다시 복구할 수 없습니다.",buttons:Ext.Msg.YESNO,icon:Ext.Msg.QUESTION,fn:function(button) {
										if (button == "yes") {
											...
											Ext.Ajax.request({
												...
												params:{"action":"trash","do":"delete","idx":idxs.join(",")}
											});
										}
									}});
								}
					...
					new Ext.Button({
						...
						handler:function() {
							Ext.Msg.show({title:"확인",msg:"휴지통을 정말 비우시겠습니까?.",buttons:Ext.Msg.YESNO,icon:Ext.Msg.QUESTION,fn:function(button) {
								if (button == "yes") {
									Ext.Msg.wait("선택한 게시물을 삭제하고 있습니다.","잠시만 기다려주십시오.");
									Ext.Ajax.request({
										url:"<?php echo $_ENV['dir']; ?>/module/release/exec/Admin.do.php",
										success:function(response) {
											var data = Ext.JSON.decode(response.responseText);
											if (data.success == true) {
												Ext.Msg.show({title:"안내",msg:"성공적으로 휴지통을 비웠습니다.",buttons:Ext.Msg.OK,icon:Ext.Msg.INFO,fn:function() {
													Ext.getCmp("ListPanel").getStore().loadPage(1);
												}});
											} else {
												Ext.Msg.show({title:"안내",msg:"서버에 이상이 있어 처리하지 못하였습니다.<br />잠시후 다시 시도해보시기 바랍니다.",buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
				...
				listeners:{
					itemdblclick:{fn:function(grid,record) {
						new Ext.Window({
							...
							layout:"fit",
							maximizable:true,
							html:'<iframe src="<?php echo $_ENV['dir']; ?>/module/release/release.php?rid='+record.data.rid+'&mode=trash&idx='+record.data.idx+'" style="width:100%; height:100%; background:#FFFFFF;" frameborder="0"></iframe>'
						}).show();
					}},
