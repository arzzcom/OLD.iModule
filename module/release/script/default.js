if (isIncludeRelease === undefined) {
	var isIncludeRelease = true;
	...
	function AutoSaveSendData(geter) {
		switch (geter) {
			case "url" :
				return "../exec/Release.do.php?action=autosave";
			break;

			case "data" :
				var isChange = false;
				var data = new Array();
				var object = document.forms["ModuleReleasePost"];
				...
				data.push(["repto",document.forms["ModuleReleasePost"].idx.value],["bid",document.forms["ModuleReleasePost"].bid.value]);
				if (isChange == true) document.getElementById("ModuleReleaseAutoSaver").save(data);
			break;
		}
	}
			...
			if (root.childNodes.length > 0) {
				...
				if (use_uploader == "TRUE") {
					...
					if (data.getAttribute("field") == "file") {
						GetSaveFile(data.getAttribute("value"),"ModuleReleasePost");
					}
				}
			}
	...
	function PostVote(idx) {
		GetHttpRequestXML(ENV.dir+"/module/release/exec/Release.do.php","action=vote&idx="+idx,"PostVoteInner");
	}
	...
	function ReplyMent(idx,midx) {
		var reply = document.forms["ModuleReleaseMent"+idx]["parent"].value;
		...
		if (midx == reply) {
			...
			document.forms["ModuleReleaseMent"+idx]["parent"].value = "";
		} else {
			rForm.innerHTML = oForm.innerHTML;
			document.forms["ModuleReleaseMent"+idx]["parent"].value = midx;
		}
		oForm.innerHTML = "";
	...
	function ListSelectCategory(text,value) {
		if (!document.forms["ModuleReleaseSearch"]["category"]) {
		...
		document.forms["ModuleReleaseSearch"]["category"].value = value;
	...
	function ListSelectSelectAnswer(text,value) {
		...
		document.forms["ModuleReleaseSearch"]["select"].value = value;
		document.forms["ModuleReleaseSearch"].submit();
	}

	function WriteSelectCategory(text,value) {
		document.forms["ModuleReleasePost"]["category"].value = value;
	}
	
	function WriteSelectLicense(text,value) {
		document.forms["ModuleReleasePost"]["license"].value = value;
	}
