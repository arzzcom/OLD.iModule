...
if ($repto != null && $autosave == null) {
	...
	for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
		...
		echo '<server>'.GetString($data[$i]['idx'].'|'.$data[$i]['filetype'].'|'.$data[$i]['filename'].'|'.$data[$i]['filesize'].($data[$i]['filetype'] == 'IMG' ? '|'.$_ENV['userfileDir'].$mRelease->thumbnail.'/'.$data[$i]['idx'].'.thm' : ''),'xml').'</server>';
		echo '</file>';
	}
} elseif ($autosave != null) {
	$data = $mDB->DBfetch($mRelease->table['autosave'],array('data','ip'),"where `tid`='$autosave'");
	if (isset($data['data']) == true && $data['ip'] == $_SERVER['REMOTE_ADDR']) {
		...
		for ($i=0, $loop=sizeof($file);$i<$loop;$i++) {
			$temp = explode('|',$file[$i]);
			$fidx = $temp[0];
			$fileData = $mDB->DBfetch($mRelease->table['file'],'*',"where `idx`=$fidx");
			echo '<file>';
