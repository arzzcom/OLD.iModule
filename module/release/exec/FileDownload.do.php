...
$idx = Request('idx');
if ($idx != null) {
	$data = $mDB->DBfetch($mRelease->table['file'],array('repto','filename','filepath','filetype','filesize'),"where `idx`='$idx'");
	$mDB->DBupdate($mRelease->table['file'],'',array('hit'=>'`hit`+1'),"where `idx`=$idx");

	if (file_exists($_ENV['userfilePath'].$mRelease->userfile.$data['filepath']) == true) {
		...
		$fp = fopen($_ENV['userfilePath'].$mRelease->userfile.$data['filepath'],'r');
		while(!feof($fp)) {
