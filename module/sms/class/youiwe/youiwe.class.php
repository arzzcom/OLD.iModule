...
/* 작성자:너나우리 모바일팀(sms@goodinternet.co.kr)
	* 작성일자:2011년 08월 23일
	* 작성목적: 본 프로그램은 너나우리 문자메세지에서 php 웹프로그램 관련 예제입다.
	*           본 예제에서는 보내는 사람의 핸드폰번호, 받는사람 핸드폰번호,전송내용 sms_main.php에서
	*           전송 받아 실제 처리하는  sms 모듈입니다.
	*/

if(substr(phpversion(), 0, 1) > 5){
	date_default_timezone_set('Asia/Seoul');
}
else if(substr(phpversion(), 0, 1) == 5 && substr(phpversion(), 2, 1) > 0){
	date_default_timezone_set('Asia/Seoul');
}
...
class SMS
{
	function SendSMS($sms_id,$sms_pwd,$snd_number,$rcv_number,$sms_content)
	{
		...
		//너나우리 웹서비스 웹 접속 포인트를 설정한다.

		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			$soapclient = new nusoap_client('http://webservice.youiwe.co.kr/SMS.v.6/ServiceSMS.asmx?WSDL',true);
	...
	function SendSMSCallBack($sms_id,$sms_pwd,$snd_number,$rcv_number,$callbackURL,$sms_content)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($sms_id.$sms_pwd.$rcv_number);//해쉬값을 생성한다.
		...
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			...
			$result = $soapclient->call('SendSMSCallBack',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.
			return $result['SendSMSCallBackResult'];
		}
		else {
			return "-50";
		}
	...
	function SendSMSCallBackReserve($sms_id,$sms_pwd,$snd_number,$rcv_number,$callbackURL,$sms_content,$reserve_date,$reserve_time,$userdefine)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($sms_id.$sms_pwd.$rcv_number);//해쉬값을 생성한다.
		...
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			...
			$result = $soapclient->call('SendSMSCallBackReserve',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.
			return $result['SendSMSCallBackReserveResult'];
		}
		else {
			return "-50";
		}
