...
if ($repto != null && $autosave == null) {
	...
	for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
		...
		echo '<server>'.GetString($data[$i]['idx'].'|'.$data[$i]['filetype'].'|'.$data[$i]['filename'].'|'.$data[$i]['filesize'].($data[$i]['filetype'] == 'IMG' ? '|'.$_ENV['dir'].$mBoard->thumbneil.'/'.$data[$i]['idx'].'.thm' : ''),'xml').'</server>';
		echo '</file>';
	}
