...
class ModuleBoard extends Module {
	...
	function __construct($bid='',$setup='') {
		...
		$this->userfile = '/userfile/board/'.date('Ym');
		$this->thumbneil = '/userfile/board/thumbneil';
		$this->skinThumbneil = '/userfile/board/thumbneil/skin';
		...
		$this->baseURL = isset($_SERVER['REDIRECT_URL']) == true && $_SERVER['REDIRECT_URL'] ? $_SERVER['REDIRECT_URL'] : $_SERVER['PHP_SELF'];
		...
		if ($bid) {
			...
			$this->setup['width'] = eregi('%',$this->setup['width']) ? $this->setup['width'] : $this->setup['width'].'px';
	...
	function GetJsonList($page,$limit,$find='') {
		...
		for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
			...
			if ($data[$i]['image'] && file_exists($_ENV['path'].$this->thumbneil.'/'.$data[$i]['image'].'.thm') == true) {
				$data[$i]['image'] = 'http://'.$_SERVER['HTTP_HOST'].$_ENV['dir'].$this->thumbneil.'/'.$data[$i]['image'].'.thm';
			} else {
				$data[$i]['image'] = '';
			}
	...
	function GetJsonCategory() {
		if ($this->setup['use_category'] == 'TRUE') {
			$category = $this->mDB->DBfetchs($this->table['category'],array('idx','category'),"where `bid`='{$this->bid}'",'order,asc');
	...
	function PrintTemplet() {
		...
		$this->mTemplet->assign('thumbneilDir',$_ENV['dir'].$this->thumbneil);
	...
	function PrintBoard() {
		...
		if (eregi('\$',$this->bid) == false && isset($this->setup['bid']) == false) {
	...
	function PrintList() {
		...
		$this->mTemplet->register_object('mBoard',$this,array('GetThumbneil'));
		$this->mTemplet->register_object('mBoard',$this,array('GetSortLink'));
		$this->PrintTemplet();
	}
	...
	function PrintView() {
		...
		if ($this->GetPermission('view') == false) return $this->PrintError('글을 읽을 수 있는 권한이 없습니다.');
	...
	function PrintWrite() {
		...
		if ($mode == 'modify') {
			...
			$extraValue = GetUnSerialize($post['extra_content']);
			...
			if ($this->setup['use_uploader'] == 'FALSE') {
				...
				for ($i=0, $loop=sizeof($files);$i<$loop;$i++) {
					...
					if ($files[$i]['filetype'] == 'IMG') $file[$i].= '|'.$_ENV['dir'].$this->thumbneil.'/'.$files[$i]['idx'].'.thm';
				}
				$file = implode(',',$file);
			}
		} else {
		...
		$categoryList = array();
		if ($this->setup['use_category'] == 'TRUE') {
			$cData = $this->mDB->DBfetchs($this->table['category'],array('idx','category','permission'),"where `bid`='{$this->bid}'",'order,asc');
			$categoryList = array();
	...
	function PrintRecently($skin,$page,$row,$limit='',$title='',$finder='') {
		...
		for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
			...
			$data[$i]['image'] = $data[$i]['image'] != '0' ? $_ENV['dir'].$this->thumbneil.'/'.$data[$i]['image'].'.thm' : '';
			$data[$i]['reg_date'] = strtotime(GetTime('c',$data[$i]['reg_date']));
	...
	function GetPermission($geter) {
		$permission = $this->setup['permission'] && is_array(GetUnSerialize($this->setup['permission'])) == true ? GetUnSerialize($this->setup['permission']) : array('list'=>true,'post'=>true,'ment'=>true,'select'=>false,'secret'=>false,'notice'=>false,'modify'=>false,'delete'=>false);
	...
	function GetThumbneil($var) {
		if (CreateDirectory($_ENV['path'].$this->skinThumbneil.'/'.$this->bid) == true) {
			if ($var['post']['image'] == '0') {
				return $var['error'];
			} elseif(file_exists($_ENV['path'].$this->skinThumbneil.'/'.$this->bid.'/'.$var['post']['idx'].'.thm') == false) {
				...
				if ($width > 0 && $height > 0 && GetThumbneil($_ENV['path'].$image['filepath'],$_ENV['path'].$this->skinThumbneil.'/'.$this->bid.'/'.$var['post']['idx'].'.thm',$width,$height) == true) {
					return $_ENV['dir'].$this->skinThumbneil.'/'.$this->bid.'/'.$var['post']['idx'].'.thm';
				} else {
					return $var['error'];
				}
			} else {
