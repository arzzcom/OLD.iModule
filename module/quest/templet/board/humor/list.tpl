...
<div id="tList">
	...
	<tr class="listrow noticerow">
		<td></td>
		{if $is_view_loopnum == true}<td class="number center">notice</td>{/if}
		<td>
		...
		{if $is_view_reg_date == true}<td class="number center">{if $time.gmt|date_format:"%Y%m%d" == $notice.reg_date|date_format:"%Y%m%d"}{$notice.reg_date|date_format:"%H:%M:%S"}{else}{$notice.reg_date|date_format:"%Y.%m.%d"}{/if}</td>{/if}
	...
	<tr class="listrow">
		...
		<td>
			...
			{if $data.field1}
			<img src="{$skinDir}/images/icon_{$data.field1}.gif" style="vetical-align:middle; margin:0px;" />
			{else}
			{if $data.category}<span class="category">[{$data.category}]</span> {/if}
			{/if}
			<a href="{$data.postlink}"{if $setup.use_mode == 'TRUE'} onclick="return OpenBoard(this);"{/if}>{$data.title|cutstring:32:true:$data.category}</a>{if $data.ment != 0} <span class="ment">[{$data.ment}{if $data.is_newment == true}+{/if}]</span>{/if}{if $data.is_new == true} <img src="{$skinDir}/images/icon_new.gif" style="vertical-align:middle;" />{/if}{if $data.is_file == true} <img src="{$skinDir}/images/icon_file.gif" style="vertical-align:middle;" />{/if}{if $data.is_image == true} <img src="{$skinDir}/images/icon_image.gif" style="vertical-align:middle;" />{/if}
		</td>
		{if $is_view_name == true}<td class="center">{$data.nickname}</td>{/if}
		{if $is_view_reg_date == true}<td class="number center">{if $time.gmt|date_format:"%Y%m%d" == $data.reg_date|date_format:"%Y%m%d"}{$data.reg_date|date_format:"%H:%M:%S"}{else}{$data.reg_date|date_format:"%Y.%m.%d"}{/if}</td>{/if}
