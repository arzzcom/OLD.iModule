...
<tr>
	...
	<td>
		...
		{if $data.image}
		<link rel="stylesheet" type="text/css" href="{$skinDir}/slider/css/style.css" />
		<div id="slider_container" class="slider_container">
			<div class="slider_loading">Loading images…</div>
			<div class="slider_slider_wrapper">
				<ul class="slider_slider">
					{foreach name=image from=$file item=image}
						{if $image.filetype == 'IMG'}<li><img src="{$moduleDir}/exec/ShowImage.do.php?idx={$image.idx}" /></li>{/if}
					{/foreach}
				</ul>
				...
				<ul class="slider_thumbnails">
					{foreach name=thumbneil from=$file item=thumbneil}
						{if $thumbneil.filetype == 'IMG'}<li><img src="{$thumbneilDir}/{$thumbneil.idx}.thm" /></li>{/if}
					{/foreach}
				</ul>
			</div>
		</div>
		...
		<script type="text/javascript" src="{$skinDir}/slider/jquery.easing.1.3.js"></script>
		<script type="text/javascript" src="{$skinDir}/slider/script.js"></script>
		{/if}
		{$data.content}
		<div class="height10"></div>
