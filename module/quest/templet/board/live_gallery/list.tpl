...
{if $categoryList}<col width="102" />{/if}{if $setup.use_select == 'TRUE'}<col width="102" />{/if}<col width="100%" />
<tr>
	{if $categoryList}
	<td></td>
	{/if}
...
<div id="tList">
	...
	<tr class="listbar">
		...
		{if $is_view_reg_date == true}<td class="text">작성일</td>{/if}
		{if $is_view_hit == true}<td class="text">조회</td>{/if}
		{if $is_view_vote == true}<td class="text">추천</td>{/if}
		{if $is_view_avgvote == true}<td class="text">평점</td>{/if}
		<td class="right"></td>
	</tr>
	</thead>
	<tbody>
	...
	<table cellpadding="0" cellspacing="0" class="layoutfixed">
	<col width="180" /><col width="33%" /><col width="180" /><col width="33%" /><col width="180" /><col width="33%" /><col width="180" />
	<tbody>
	{foreach name=list from=$data item=data}
		{if $smarty.foreach.list.index % 4 == 0}<tr class="listrow">{else}<td></td>{/if}
		<td>
			<div class="height10"></div>
			<div class="thumbneil" style="background-image:url({mBoard->GetThumbneil post=$data width=176 height=0 min_height=110 error=$skinDir|cat:'/images/noimage.gif'})" onclick="location.href='{$data.postlink}';"></div>
			<table cellpadding="0" cellspacing="0" class="layoutfixed">
			<col width="30" /><col width="100%" />
			<tr>
				<td class="number">{if $data.is_read == true}<img src="{$skinDir}/images/icon_read.gif" />{else}{$data.loopnum}.{/if}</td>
				<td class="right">{$data.nickname}</td>
			</tr>
			</table>
			{if $data.category}<div class="category">[{$data.category}]</div>{/if}
			<div class="title"><a href="{$data.postlink}"{if $setup.use_mode == 'TRUE'} onclick="return OpenBoard(this);"{/if}>{$data.title|cutstring:20:true:$data.category}</a>{if $data.is_new == true} <img src="{$skinDir}/images/icon_new.gif" style="vertical-align:middle;" />{/if}</div>
			<div class="info">
				<span class="ment">{$data.hit}</span> hits, <span class="ment">{$data.ment}{if $data.is_newment == true}+{/if}</span> replys, <span class="ment">{$data.vote}</span> votes</span><br />
				At {if $time.server|date_format:"%Y%m%d" == $data.reg_date|date_format:"%Y%m%d"}{$data.reg_date|date_format:"%H:%M:%S"}{else}{$data.reg_date|date_format:"%Y.%m.%d"}{/if}
			</div>
	...
	{if $smarty.foreach.list.total%4 != 0}
	{section name=idx start=$smarty.foreach.list.total%4-4 step=1 loop=4}
	<td></td><td></td>
	{/section}
	</tr>
	{/if}
	<tr class="listrowbar"><td colspan="7"></td></tr>
	</tbody>
	</table>
