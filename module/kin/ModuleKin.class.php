<?php
class ModuleKin extends Module {
	public $table = array();
	public $bid = null;
	...
	function __construct() {
		...
		$this->table['question'] = $_ENV['code'].'_kin_question_table';
		$this->table['answer'] = $_ENV['code'].'_kin_answer_table';
		$this->table['ment'] = $_ENV['code'].'_kin_ment_table';
		$this->table['thanks'] = $_ENV['code'].'_kin_thanks_table';
		$this->table['category'] = $_ENV['code'].'_kin_category_table';
		$this->table['file'] = $_ENV['code'].'_kin_file_table';
		$this->table['log'] = $_ENV['code'].'_kin_log_table';
		$this->table['autosave'] = $_ENV['code'].'_kin_autosave_table';
		$this->userfile = '/userfile/kin/'.date('Ym');
		$this->thumbnail = '/userfile/kin/thumbnail';
		$this->skinThumbnail = '/userfile/board/thumbnail/skin';

		parent::__construct('kin');
		
		$this->find = "where `is_delete`='FALSE'";
		$this->idx = $this->mDB->AntiInjection(Request('idx'));
		$this->mode = Request('mode') ? Request('mode') : ($this->idx == null ? 'list' : 'view');

		$this->totalpost = $this->mDB->DBcount($this->table['question'],$this->find);
		...
		$this->module = array('skin'=>'default','listnum'=>'30','pagenum'=>'11','point_select'=>50,'point_question'=>10,'use_uploader'=>'TRUE');
		
		$this->point = array('select'=>$this->module['point_select']);

		$this->skinPath = $this->modulePath.'/templet/kin/'.$this->module['skin'];
		$this->skinDir = $this->moduleDir.'/templet/kin/'.$this->module['skin'];
	}
	...
	function GetUserStatus($mno) {
		if ($mno == '0' || !$mno) return array('question'=>'0','complete'=>'0.00%','answer'=>'0','selected'=>'0.00%');
		
		$question = $this->mDB->DBcount($this->table['question'],"where `mno`='$mno' and `is_delete`='FALSE'");
		$complete = $question > 0 ? sprintf('%0.2f',$this->mDB->DBcount($this->table['question'],"where `mno`='$mno' and `is_complete`='TRUE'")/$question*100) : '0.00';
		
		$answer = $this->mDB->DBcount($this->table['answer'],"where `mno`='$mno' and `is_delete`='FALSE'");
		$selected = $answer > 0 ? sprintf('%0.2f',$this->mDB->DBcount($this->table['answer'],"where `mno`='$mno' and `is_select`='TRUE'")/$answer*100) : '0.00';
	...
	function GetCategoryList($parent='') {
		$find = strlen($parent) > 0 ? "where `parent`='$parent'" : '';
		return $this->mDB->DBfetchs($this->table['category'],array('idx','category'),$find,'sort,asc');
	}
	...
	function GetContent($content) {
		...
		if ($this->module['mobile'] == true) $content = '<div class="smartOutputMobile">'.$content.'</div>';
			...
			for ($i=0, $loop=sizeof($match[0]);$i<$loop;$i++) {
				...
				if ($file['filetype'] == 'MOV') {
					if (preg_match('/(Safari|Chrome)/',$_SERVER['HTTP_USER_AGENT']) == true) {
						$content = str_replace($match[0][$i],'<video src="'.$_ENV['dir'].$file['filepath'].'" '.$match[3][$i].' controls="controls" preload="preload"></video>',$content);
					} else {
						$content = str_replace($match[0][$i],'<embed src="'.$_ENV['dir'].$file['filepath'].'" '.$match[3][$i].' autostart="false" showcontrols="true" showstatusbar="true" controller="true"></embed>',$content);
					}
				}
			}
		}
	...
	function PrintTemplet() {
		...
		$this->link['prevURL'] = isset($this->module['prevURL']) == true ? $this->module['prevURL'] : '';
		...
		$this->link['write_question'] = $this->baseURL.$this->GetQueryString(array('sort'=>'','dir'=>'','key'=>'','keyword'=>'','p'=>'','mode'=>'write_question','idx'=>''));
		...
		$this->mTemplet->assign('setup',$this->module);
	...
	function PrintHeader($title='') {
		...
		if ($this->module['mobile'] == true) {
			if ($_ENV['isHeaderIncluded'] == false) {
				GetDefaultHeader($title ? $title : $this->module['title'],'',array(
			...
			if ($this->bid) {
				...
				echo '<div class="ModuleBoard" style="width:'.$this->module['width'].'">'."\n";
			}
		} else {
			if ($_ENV['isHeaderIncluded'] == false) {
				GetDefaultHeader($title ? $title : $this->module['title']);
			}
			...
			echo '<link rel="stylesheet" href="'.$this->skinDir.'/style.css" type="text/css" title="style" />'."\n";
			echo '<script type="text/javascript" src="'.$this->skinDir.'/script.js"></script>'."\n";
			echo '<div class="ModuleKin" style="width:'.$this->module['width'].'">'."\n";
		}
	}
	...
	// 답변 선택창 출력
	function PrintSelectAnswer($idx) {
		$this->PrintHeader('답변 채택하기');
		
		$answer = $this->mDB->DBfetch($this->table['answer'],array('mno','repto'),"where `idx`='$idx'");
		$question = $this->mDB->DBfetch($this->table['question'],array('mno','point','is_complete','is_select'),"where `idx`='{$answer['repto']}'");
		
		$is_password = false;
		if ($this->GetPermission('select') == false) {
			if ($question['mno'] == '0') {
				$is_password = true;
			} else if ($question['mno'] != $this->member['idx']) {
				Alertbox('답변을 채택할 권한이 없습니다.',2);
			}
		}
		
		if (file_exists($this->skinPath.'/select.tpl') == true) {
			$this->mTemplet = new Templet($this->skinPath.'/select.tpl');
		} else {
			$this->mTemplet = new Templet($this->modulePath.'/templet/select.tpl');
		}

		$formStart = '<form method="post" action="'.$this->moduleDir.'/exec/Kin.do.php" target="execFrame">'."\n";
		$formStart.= '<input type="hidden" name="action" value="select" />'."\n";
		$formStart.= '<input type="hidden" name="idx" value="'.$idx.'" />'."\n";
		$formEnd.= '</form>'."\n".'<iframe name="execFrame" style="display:none;"></iframe>'."\n";

		$this->point['gift'] = floor($question['point']/2);

		$this->mTemplet->assign('formStart',$formStart);
		$this->mTemplet->assign('formEnd',$formEnd);
		$this->mTemplet->assign('is_password',$is_password);
		$this->mTemplet->assign('point',$this->point);
		$this->PrintTemplet();
	...
	// 질문 마감
	function PrintCompleteQuestion($idx) {
		$this->PrintHeader('질문 마감하기');
		
		$question = $this->mDB->DBfetch($this->table['question'],array('mno','point','is_complete','is_select'),"where `idx`='$idx'");
		...
		if ($this->GetPermission('select') == false) {
			if ($question['mno'] == '0') {
				...
				Alertbox('질문을 마감할 권한이 없습니다.',2);
			}
		}
		
		if (file_exists($this->skinPath.'/select.tpl') == true) {
			$this->mTemplet = new Templet($this->skinPath.'/complete.tpl');
		} else {
			$this->mTemplet = new Templet($this->modulePath.'/templet/complete.tpl');
		}

		$formStart = '<form method="post" action="'.$this->moduleDir.'/exec/Kin.do.php" target="execFrame">'."\n";
		$formStart.= '<input type="hidden" name="action" value="complete" />'."\n";
		$formStart.= '<input type="hidden" name="idx" value="'.$idx.'" />'."\n";
		$formEnd.= '</form>'."\n".'<iframe name="execFrame" style="display:none;"></iframe>'."\n";
	...
	function GetAnswer($idx) {
		$select = $this->mDB->DBfetch($this->table['answer'],'*',"where `repto`=$idx and `is_select`='TRUE'");
		$data = $this->mDB->DBfetchs($this->table['answer'],'*',"where `repto`=$idx and `is_delete`='FALSE' and `is_select`='FALSE'",'idx,asc');

		if (isset($select['idx']) == true) array_unshift($data, $select);
		...
		for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
			...
			if ($data[$i]['is_select'] == 'TRUE') {
				$thanks = $this->mDB->DBfetch($this->table['thanks'],array('message'),"where `ano`='{$data[$i]['idx']}'");
				$data[$i]['thanks'] = nl2br($thanks['message']);
			}

			$data[$i]['link'] = array();
			$data[$i]['link']['modify'] = $this->baseURL.$this->GetQueryString(array('mode'=>'modify_answer','repto'=>$idx,'idx'=>$data[$i]['idx'],'p'=>''));
			$data[$i]['link']['delete'] = $this->baseURL.$this->GetQueryString(array('mode'=>'delete_answer','idx'=>$data[$i]['idx'],'p'=>''));

			$data[$i]['action'] = array();
			$data[$i]['action']['reply'] = 'ReplyMent('.$idx.','.$data[$i]['idx'].');';
			$data[$i]['action']['select'] = 'SelectAnswer('.$data[$i]['idx'].');';
			$data[$i]['action']['ment'] = 'ToggleMent(\'answer\','.$data[$i]['idx'].');';

			$data[$i]['is_select'] = $data[$i]['is_select'] == 'TRUE';
			$data[$i]['is_complete'] = $data[$i]['is_complete'] == 'TRUE';
			
			$data[$i]['user'] = $this->GetUserStatus($data[$i]['mno']);
			$data[$i]['ment'] = '<span id="KinMentNumber-answer-'.$data[$i]['idx'].'">'.number_format($data[$i]['ment']).'</span>';
			
			$data[$i]['permission'] = array();
			$data[$i]['permission']['modify'] = $this->GetPermission('modify') == true || ($data[$i]['mno'] == '0' || $data[$i]['mno'] == $this->member['idx']);
			$data[$i]['permission']['delete'] = $this->GetPermission('delete') == true || ($data[$i]['mno'] == '0' || $data[$i]['mno'] == $this->member['idx']);
			...
			$data[$i]['mentlist'] = '<div id="KinMentArea-answer-'.$data[$i]['idx'].'" style="display:none;"><iframe id="KinMentFrame-answer-'.$data[$i]['idx'].'" frameborder="0" style="width:100%; height:5px;"></iframe></div>';
		}
	...
	// 댓글처리
	function PrintMent($type,$idx) {
		$this->PrintHeader('댓글보기');
		
		$data = $this->mDB->DBfetchs($this->table['ment'],'*',"where `type`='".strtoupper($type)."' and `repto`='$idx'");
		...
		for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
			...
			$data[$i]['permission']['delete'] = $this->GetPermission('ment') == true || ($data[$i]['mno'] == '0' || $data[$i]['mno'] == $this->member['idx']);
		}
		...
		$actionTarget = 'execMentWriteFrame'.$idx;
		$formStart = '<form name="ModuleKinMent'.$idx.'" method="post" action="'.$this->moduleDir.'/exec/Kin.do.php" target="'.$actionTarget.'" onsubmit="return CheckMent(this)">'."\n";
		$formStart.= '<input type="hidden" name="action" value="ment" />'."\n";
		$formStart.= '<input type="hidden" name="type" value="'.$type.'" />'."\n";
		$formStart.= '<input type="hidden" name="repto" value="'.$idx.'" />'."\n";
		$formEnd = '</form>'."\n".'<iframe name="'.$actionTarget.'" style="display:none;"></iframe>'."\n";

		$this->mTemplet = new Templet($this->skinPath.'/ment.tpl');
		$this->mTemplet->assign('member',$this->mMember->GetMemberInfo());
		$this->mTemplet->assign('formStart',$formStart);
		$this->mTemplet->assign('formEnd',$formEnd);
		...
		echo '<script type="text/javascript">addEvent(window,"load",function() { AutoResizeKinMentArea("'.$type.'",'.$idx.','.$loop.'); });</script>';
	...
	function PrintKin() {
		...
		switch ($this->mode) {
			...
			case 'write_question' :
				$this->PrintWriteQuestion();
			break;

			case 'write_answer' :
				$this->PrintWriteAnswer();
			break;
	...
	function PrintList() {
		...
		if ($this->idx) $find = str_replace('where ','where ((',$find).') or `idx`=\''.$this->idx.'\')';
		$listnum = $this->module['listnum'];
		$pagenum = $this->module['pagenum'];
		$p = is_numeric(Request('p')) == true && Request('p') > 0 ? Request('p') : 1;

		$totalpost = $this->mDB->DBcount($this->table['question'],$find);
		...
		if ($this->idx != null) {
			...
			$prevNum = $this->mDB->DBcount($this->table['question'],$prevFind);
			$p = ceil($prevNum/$listnum);
		}
		$orderer = $sort.','.$dir;
		$limiter = ($p-1)*$listnum.','.$listnum;

		$data = $this->mDB->DBfetchs($this->table['question'],array('idx','category1','category2','category3','mno','name','email','title','image','reg_date','hit','answer','last_answer','is_select','is_complete','is_rewrite','is_secret','is_mobile','is_hidename','point'),$find,$orderer,$limiter);
		...
		for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
			...
			$data[$i]['postlink'] = $this->module['use_mode'] == 'TRUE' ? $this->moduleDir.'/board.php?bid='.$data[$i]['bid'].'&amp;mode=view&amp;idx='.$data[$i]['idx'] : $this->baseURL.$this->GetQueryString(array('mode'=>'view','idx'=>$data[$i]['idx']));
			$data[$i]['reg_date'] = strtotime(GetTime('c',$data[$i]['reg_date']));
			...
			$data[$i]['is_newanswer'] = $data[$i]['last_answer'] > GetGMT()-60*60*24*10;

			if ($this->module['use_category'] == 'TRUE' && $data[$i]['category'] != '0') {
				$data[$i]['category'] = $this->GetCategoryName($data[$i]['category']);
			} else {
				$data[$i]['category'] = '';
			}

			if ($this->module['use_mode'] == 'TRUE') $data[$i]['category'] = $this->GetBoardTitle($data[$i]['bid']);
		}
		...
		$categoryName = '';
		$categoryList = array();
		if ($this->module['use_category'] == 'TRUE') {
	...
	function PrintView() {
		...
		$find = $this->find;
		$find.= " and `idx`='$idx'";
		$data = $this->mDB->DBfetch($this->table['question'],'*',$find);
		...
		if ($this->mDB->DBcount($this->table['log'],"where `repto`=$idx and `type`='HIT' and (`mno`={$this->member['idx']} or `ip`='".$_SERVER['REMOTE_ADDR']."')") == 0) {
			$this->mDB->DBupdate($this->table['question'],'',array('hit'=>'`hit`+1'),"where `idx`='$idx'");
		...
		if ($this->module['use_category'] == 'TRUE' && $data['category'] != '0') {
			$data['category'] = $this->GetCategoryName($data['category']);
		} else {
			$data['category'] = '';
		}
		
		$data['is_complete'] = $data['is_complete'] == 'TRUE';
		$data['ment'] = '<span id="KinMentNumber-question-'.$data['idx'].'">'.number_format($data['ment']).'</span>';

		$permission = array();
		$permission['answer'] = $this->GetPermission('answer') == true && ($data['mno'] == '0' || $data['mno'] != $this->member['idx']);
		if ($permission['answer'] == true) {
			if ($this->mMember->IsLogged() == true) {
				$permission['answer'] = $this->mDB->DBcount($this->table['answer'],"where `repto`='{$data['idx']}' and `mno`='{$this->member['idx']}'") == 0;
			} else {
				$permission['answer'] = $this->mDB->DBcount($this->table['answer'],"where `repto`='{$data['idx']}' and `ip`='{$_SERVER['REMOTE_ADDR']}'") == 0;
			}
		}
		
		$permission['modify'] = $this->GetPermission('modify') == true || ($data['mno'] == '0' || $data['mno'] == $this->member['idx']);
		$permission['ment'] = $this->GetPermission('ment') == true && $data['is_ment'] == 'TRUE';
		$permission['delete'] = $this->GetPermission('delete') || ($data['mno'] == '0' || $data['mno'] == $this->member['idx']);
		$permission['select'] = false;
		if ($data['is_complete'] == false && ($this->GetPermission('select') == true || ($data['mno'] != '0' && $data['mno'] == $this->member['idx']) || $data['mno'] == '0')) {
			$permission['select'] = true;
		}

		$file = $this->mDB->DBfetchs($this->table['file'],'*',"where `type`='QUESTION' and `repto`=$idx");

		for ($i=0, $loop=sizeof($file);$i<$loop;$i++) {
			if ($this->module['mobile'] == true && $file[$i]['filetype'] == 'IMG') {
		...
		$answer = $this->GetAnswer($idx);
		$mentlist = '<div id="KinMentArea-question-'.$idx.'" style="display:none;"><iframe id="KinMentFrame-question-'.$idx.'" frameborder="0" style="width:100%; height:5px;"></iframe></div>';
		$this->action['vote'] = 'PostVote('.$idx.')';
		$this->link['postment'] = $this->baseURL.GetQueryString(array('mode'=>'ment_write','repto'=>$idx,'idx'=>''));
		$this->link['answer'] = $this->baseURL.GetQueryString(array('mode'=>'write_answer','repto'=>$idx,'idx'=>''));
		$this->link['modify_question'] = $this->baseURL.GetQueryString(array('mode'=>'modify_question','idx'=>$idx));
		$this->link['delete_question'] = $this->baseURL.GetQueryString(array('mode'=>'delete_question','idx'=>$idx));
		$this->action['ment'] = 'ToggleMent(\'question\','.$idx.');';
		$this->action['complete'] = 'CompleteQuestion('.$idx.');';

		$this->mTemplet = new Templet($this->skinPath.'/view.tpl');
		$this->mTemplet->assign('data',$data);
		$this->mTemplet->assign('user',$this->GetUserStatus($data['mno']));
		$this->mTemplet->assign('file',$file);
		$this->mTemplet->assign('answer',$answer);
		$this->mTemplet->assign('mentlist',$mentlist);
		$this->mTemplet->assign('permission',$permission);

		$this->PrintTemplet();

		echo "\n".'<iframe name="downloadFrame" style="display:none;"></iframe><iframe name="execFrame" style="display:none;"></iframe>'."\n";
		if ($this->module['view_alllist'] == 'TRUE' && $this->module['mobile'] == false) {
...
//		$ment = $this->GetMent($idx);
	...
	// 쓰기 출력
	function PrintWriteQuestion() {
		if ($this->module['use_mode'] == 'TRUE') return $this->PrintError('현재 모드에서는 게시물을 작성하거나 수정할 수 없습니다.');
		$idx = $this->idx;
		$mode = Request('mode') == 'modify_question' && $idx != null ? 'modify' : 'post';

		if ($mode == 'modify') {
			$post = $this->mDB->DBfetch($this->table['question'],'*',"where `idx`='$idx'");
			if ($post['answer'] > 0) return $this->PrintError('답변이 등록된 게시물은 수정할 수 없습니다.');
			...
			$post['is_hidename'] = $post['is_hidename'] == 'TRUE';
			$post['is_msg'] = $post['is_msg'] == 'TRUE';
			$post['is_email'] = $post['is_email'] == 'TRUE';
			$post['is_agree'] = true;

			$password = ArzzEncoder(Request('password'));
			$image = $post['image'];

			if ($this->module['use_uploader'] == 'FALSE') {
			...
			$post = array('name'=>GetString(Request('iModuleKinName','cookie'),'ext'),'category'=>Request('category'),'title'=>'','content'=>'','email'=>GetString(Request('iModuleKinEmail','cookie'),'ext'),'is_secret'=>false,'is_msg'=>true,'is_email'=>false);
			$password = '';
			$image = '';
		}
		$actionTarget = 'postFrame'.rand(100,999);
		$formStart = '<form name="ModuleKinPost" method="post" action="'.$this->moduleDir.'/exec/Kin.do.php" target="'.$actionTarget.'" onsubmit="return CheckPost(this)" enctype="multipart/form-data">'."\n";
		if ($this->module['mobile'] == true) $formStart.= '<input type="hidden" name="is_mobile" value="TRUE" />';
		$formStart.= '<input type="hidden" name="action" value="question" />'."\n";
		...
		$formStart.= '<script type="text/javascript">GetEmbed("ModuleKinAutoSaver","'.$this->moduleDir.'/flash/AutoSaver.swf?rnd='.time().'",1,1,"funcname=AutoSaveSendData&resultname=AutoSaveComplete");</script>'."\n";
		$formEnd = '</form>'."\n".'<iframe name="'.$actionTarget.'" style="display:none;"></iframe>'."\n";
		$formEnd.= '<div id="AutoSaverAlertBox" style="display:none;"></div>'."\n";

		if ($mode == 'modify' && $this->module['use_uploader'] == 'TRUE') $formEnd.= '<script type="text/javascript">AzUploaderComponent.load("repto='.$idx.'");</script>';

		$autosaveFind = "where `type`='QUESTION'";
		$autosaveFind.= $idx != null ? " and `repto`=$idx" : '';
		$autosaveFind.= " and `ip`='".$_SERVER['REMOTE_ADDR']."'";
		$autosave = $this->mDB->DBfetch($this->table['autosave'],array('tid','type','reg_date'),$autosaveFind);

		if (isset($autosave['tid']) == true) $formEnd.= '<script type="text/javascript">GetAutoSave("question","'.$autosave['tid'].'","'.GetTime('Y년 m월 d일 H시 i분',$autosave['reg_date']).'","'.$this->moduleDir.'/exec/Ajax.get.php","'.$this->module['use_uploader'].'");</script>'."\n";
		...
		$categoryList1 = $this->GetCategoryList(0);
		$categoryName1 = $this->GetCategoryName($category1);
		if ($post['category1'] > 0) {
			$categoryList2 = $this->GetCategoryList($post['category1']);
			$categoryName2 = $this->GetCategoryName($post['category1']);
		} else {
			$categoryList2 = array();
			$categoryName2 = '';
		}
		if ($post['category2'] > 0) {
			$categoryList3 = $this->GetCategoryList($post['category2']);
			$categoryName3 = $this->GetCategoryName($post['category2']);
		} else {
			$categoryList3 = array();
			$categoryName3 = '';
		}
		...
		$this->mTemplet = new Templet($this->skinPath.'/write_question.tpl');
		$this->mTemplet->assign('formStart',$formStart);
		$this->mTemplet->assign('formEnd',$formEnd);
		$this->mTemplet->assign('formName','ModuleKinPost');
		...
		$this->mTemplet->register_object('mKin',$this,array('PrintUploader','PrintWysiwyg'));
		$this->PrintTemplet();
	}

	function PrintWriteAnswer() {
		$idx = Request('idx');
		$repto = Request('repto');
		$mode = Request('mode') == 'modify_answer' && $idx != null ? 'modify' : 'post';
		...
		if ($data['is_secret'] == 'TRUE' && $this->GetPermission('secret') == false) {
			if ($data['mno'] != '0' && $data['mno'] != $this->member['idx']) {
				...
				$password = Request('password');
				if ($password == null) {
					return $this->PrintInputPassword('비밀글에 답변을 등록하기 위해 게시물의 패스워드를 입력하여 주십시오.',$this->baseURL.$this->GetQueryString());
				} else {
					if (md5($password) != $data['password']) {
						return $this->PrintInputPassword('패스워드가 일치하지 않습니다.<br />패스워드를 정확히 입력하여 주십시오.',$this->baseURL.$this->GetQueryString());
					}
		...
		if ($this->GetPermission('answer') == false) {
			return $this->PrintError('답변할 수 있는 권한이 없습니다');
		}
		...
		if ($data['is_complete'] == true) return $this->PrintError('이미 마감된 질문으로 추가답변이 불가능합니다');

		$file = $this->mDB->DBfetchs($this->table['file'],'*',"where `type`='QUESTION' and `repto`=$repto");
		...
		// 원본글
		if ($mode == 'modify') {
			$post = $this->mDB->DBfetch($this->table['answer'],'*',"where `idx`='$idx'");
			...
			$image = $post['image'];
			if ($this->module['use_uploader'] == 'FALSE') {
				$files = $this->mDB->DBfetchs($this->table['file'],'*',"where `type`='ANSWER' and `repto`=$idx",'idx,asc');
				$file = array();
				for ($i=0, $loop=sizeof($files);$i<$loop;$i++) {
					$file[$i] = $files[$i]['idx'].'|'.$files[$i]['filetype'].'|'.$files[$i]['filename'].'|'.$files[$i]['filesize'].'|'.$files[$i]['wysiwyg'];
					if ($files[$i]['filetype'] == 'IMG') $file[$i].= '|'.$_ENV['dir'].$this->thumbnail.'/'.$files[$i]['idx'].'.thm';
			...
			if ($this->GetPermission('answer') == false) return $this->PrintError('글을 작성할 수 있는 권한이 없습니다.');
			$post = array('name'=>GetString(Request('iModuleKinName','cookie'),'ext'),'title'=>'Re : '.$data['title'],'content'=>'','email'=>GetString(Request('iModuleKinEmail','cookie'),'ext'),'is_hidename'=>false,'is_agree'=>false,);
			$password = '';
			$image = '';
		}
		...
		$formStart = '<form name="ModuleKinPost" method="post" action="'.$this->moduleDir.'/exec/Kin.do.php" target="'.$actionTarget.'" onsubmit="return CheckPost(this)" enctype="multipart/form-data">'."\n";
		if ($this->module['mobile'] == true) $formStart.= '<input type="hidden" name="is_mobile" value="TRUE" />';
		$formStart.= '<input type="hidden" name="action" value="answer" />'."\n";
		$formStart.= '<input type="hidden" name="mode" value="'.$mode.'" />'."\n";
		$formStart.= '<input type="hidden" name="idx" value="'.$idx.'" />'."\n";
		...
		$this->mTemplet = new Templet($this->skinPath.'/write_answer.tpl');
		$this->mTemplet->assign('data',$data);
		$this->mTemplet->assign('user',$this->GetUserStatus($data['mno']));
		$this->mTemplet->assign('file',$file);
		$this->mTemplet->assign('formStart',$formStart);
	...
	// 삭제출력
	function PrintDelete($mode) {
		$idx = $this->idx;
		$find = "where `idx`='$idx'";
		$data = $this->mDB->DBfetch($this->table[$mode],'*',$find);

		if ($this->GetPermission('delete') == false) {
			if ($data['mno'] == '0') {
				$this->PrintInputPassword('게시물을 삭제하기 위하여 패스워드를 입력하여 주십시오.',$this->moduleDir.'/exec/Kin.do.php?action=delete&mode='.$mode.'&idx='.$idx,'execFrame');
			} elseif ($data['mno'] == $this->member['idx']) {
				$this->PrintConfirm('게시물을 삭제하시겠습니까?',$this->moduleDir.'/exec/Kin.do.php?action=delete&mode='.$mode.'&idx='.$idx,$this->baseURL.$this->GetQueryString(array('mode'=>'view')),'execFrame');
			} else {
				return $this->PrintError('글을 삭제할 권한이 없습니다.');
			}
		} else {
			$this->PrintConfirm('게시물을 삭제하시겠습니까?',$this->moduleDir.'/exec/Board.do.php?action=delete&mode='.$mode.'&idx='.$idx,$this->baseURL.$this->GetQueryString(array('mode'=>'view')),'execFrame');
		}
	}
	...
	function PrintMentWrite() {
		...
		if ($mode == 'ment_modify' && $this->module['use_uploader'] == 'TRUE') $formEnd.= '<script type="text/javascript">AzUploaderComponent.load("repto='.$idx.'");</script>';
	...
	function PrintRecently($skin,$page,$row,$limit='',$title='',$finder='') {
		...
		$title = $title ? $title : $this->module['title'];
		$find = $this->find;
		...
		for ($i=0, $loop=sizeof($data);$i<$loop;$i++) {
			...
			$data[$i]['postlink'] = $page.(ereg('\?',$page) == true ? '&amp;' : '?').'mode=view&amp;idx='.$data[$i]['idx'];
			$data[$i]['image'] = $data[$i]['image'] != '0' ? $_ENV['dir'].$this->thumbnail.'/'.$data[$i]['image'].'.thm' : '';
			$data[$i]['reg_date'] = strtotime(GetTime('c',$data[$i]['reg_date']));
	...
	function PrintUploader($var) {
		$use_uploader = false;
		if ($this->module['use_uploader'] == 'TRUE') {
		...
		if ($use_uploader == true) {
			$this->mUploader->SetCaller('kin',$this);
	...
	function GetPermission($geter) {
		$permission = $this->module['permission'] && is_array(GetUnSerialize($this->module['permission'])) == true ? GetUnSerialize($this->module['permission']) : array('list'=>true,'post'=>true,'ment'=>true,'select'=>false,'secret'=>false,'notice'=>false,'modify'=>false,'delete'=>false);
	...
	function GetConfig($geter) {
		return $this->module[$geter];
	}

	function GetThumbnail($var) {
		if (CreateDirectory($_ENV['path'].$this->skinThumbnail.'/'.$this->bid) == true) {
			if ($var['post']['image'] == '0') {
				return $var['error'];
			} elseif(file_exists($_ENV['path'].$this->skinThumbnail.'/'.$this->bid.'/'.$var['post']['idx'].'.thm') == false) {
				...
				if ($var['width'] > 0 && $var['height'] > 0) {
					...
					$check = @getimagesize($_ENV['path'].$image['filepath']);
					$width = $var['width'];
				...
				if ($width > 0 && $height > 0 && GetThumbnail($_ENV['path'].$image['filepath'],$_ENV['path'].$this->skinThumbnail.'/'.$this->bid.'/'.$var['post']['idx'].'.thm',$width,$height) == true) {
					return $_ENV['dir'].$this->skinThumbnail.'/'.$this->bid.'/'.$var['post']['idx'].'.thm';
				} else {
					return $var['error'];
				}
			} else {
	...
	function FileDelete($idx) {
		...
		if ($this->mDB->DBcount($this->table['file'],"where `filepath`='{$file['filepath']}'") == 0) {
			@unlink($_ENV['path'].$file['filepath']);
		}
	}
