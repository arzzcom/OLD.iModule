...
$idx = Request('idx');
if ($idx != null) {
	$data = $mDB->DBfetch($mKin->table['file'],array('repto','filename','filepath','filetype','filesize'),"where `idx`=$idx");
	$mDB->DBupdate($mKin->table['file'],'',array('hit'=>'`hit`+1'),"where `idx`=$idx");
