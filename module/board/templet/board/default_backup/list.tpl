...
<div id="tList">
	...
	<tr class="listrow">
		...
		<td>
			...
			{if $data.is_select == 'TRUE'}<span class="select">답변채택</span>{elseif $data.is_select == 'COMPLETE'}<span class="select">미해결완료</span>{/if}
			{if $data.category}<span class="category">[{$data.category}]</span> {/if}<a href="{$data.postlink}">{$data.title|cutstring:32:true:$data.category}</a>{if $data.ment != 0} <span class="ment">[{$data.ment}{if $data.is_newment == true}+{/if}]</span>{/if}{if $data.is_new == true} <img src="{$skinDir}/images/icon_new.gif" style="vertical-align:middle;" />{/if}{if $data.is_file == true} <img src="{$skinDir}/images/icon_file.gif" style="vertical-align:middle;" />{/if}{if $data.is_image == true} <img src="{$skinDir}/images/icon_image.gif" style="vertical-align:middle;" />{/if}
		</td>
