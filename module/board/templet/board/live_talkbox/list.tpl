...
<div id="tList">
	...
	<tr class="listrow">
		...
		<td>
			<table cellpadding="0" cellspacing="0" class="layoutfixed">
			{if $data.field1}<col width="65" />{/if}<col width="100%" />
			<tr>
				{if $data.field1}<td><img src="{$skinDir}/images/icon_{$data.field1}.gif" style="vetical-align:middle; margin:0px;" /></td>{/if}
				<td nowrap="nowrap">
					...
					{if $data.category}<span class="category">[{$data.category}]</span> {/if}<a href="{$data.postlink}">{if $data.field1}{$data.title|cutstring:32:true:$data.category}{else}{$data.title|cutstring:24:true:$data.category}{/if}</a>{if $data.ment != 0} <span class="ment">[{$data.ment}{if $data.is_newment == true}+{/if}]</span>{/if}{if $data.is_new == true} <img src="{$skinDir}/images/icon_new.gif" style="vertical-align:middle;" />{/if}{if $data.is_file == true} <img src="{$skinDir}/images/icon_file.gif" style="vertical-align:middle;" />{/if}{if $data.is_image == true} <img src="{$skinDir}/images/icon_image.gif" style="vertical-align:middle;" />{/if}
				</td>
			</tr>
			</table>
		</td>
