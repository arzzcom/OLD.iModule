<article>
	<header>
		...
		<tr>
			<td colspan="6" class="sectionTitle">이미지 보기</td>
	...
	<link rel="stylesheet" href="{$skinDir}/galleria/themes/azur/galleria.azur.css">
	...
	<div id="galleria" style="width:100%; height:500px;">
		{foreach name=image from=$file item=image}
			{if $image.filetype == "IMG"}
			<a href="{$moduleDir}/exec/ShowImage.do.php?idx={$image.idx}"><img src="{$thumbnailDir}/{$image.idx}.thm", data-title="{$image.filename} ({$image.filesize})" /></a>
			{/if}
		{/foreach}
	</div>
