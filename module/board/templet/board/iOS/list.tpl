...
<div id="content" class="whiteBg">
	<ul id="main" title="Tamrin's Link" selected="true">
		...
		<li><a href="{$data.postlink}"><span class="listTitle">{if $data.is_image == true}<img src="{$skinDir}/images/icon_image.gif" style="vertical-align:middle; width:16px; height:16px; margin-right:5px;" />{/if}{$data.title}</span><span class="listMent">[{if $data.is_newment == true}+{/if}{$data.ment}]</span></a></li>
		{/foreach}
	</ul>
	...
	<div class="searchbox">
		...
		<tr>
			<td><input type="search" name="keyword" value="{$keyword}" /></td>
