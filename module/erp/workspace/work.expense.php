		...
		items:[
			new Ext.grid.EditorGridPanel({
						...
						handler:function() {
							...
							if (idxs.length > 0) {
								Ext.Ajax.request({
									...
									headers:{},
									params:{"action":"work","do":"expense","wno":"<?php echo $this->wno; ?>","mode":"delete","idx":idxs.join(",")}
								});
							} else {
								Ext.Msg.show({title:"안내",msg:"성공적으로 삭제되었습니다.",buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO});
							}
					...
					new Ext.Button({
						...
						handler:function() {
							...
							Ext.Ajax.request({
								url:"<?php echo $this->moduleDir; ?>/exec/Workspace.do.php",
								success:function() {
									Ext.Msg.show({title:"안내",msg:"경비내역이 성공적으로 저장되었습니다.",buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO,animEl:"SaveButton"});
								...
								headers:{},
								params:{"action":"work","do":"expense","wno":"<?php echo $this->wno; ?>","mode":"modify","data":data,"date":new Date(Ext.getCmp("today").getValue()).format("Y-m-d")}
							});
						}
					})
				],
				...
				store:new Ext.data.GroupingStore({
					...
					baseParams:{"wno":"<?php echo $this->wno; ?>","action":"work","get":"expense","date":new Date(Ext.getCmp("today").getValue()).format("Y-m-d")}
				}),
				trackMouseOver:true,
