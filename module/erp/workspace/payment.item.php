			...
			new Ext.grid.EditorGridPanel({
				...
				tbar:[
					...
					new Ext.Button({
						...
						handler:function() {
							...
							GridInsertRow(Ext.getCmp("PaymentList"),{"idx":"0","is_new":"TRUE","group":" ","date":date,"order_ea":"0,0,0,0","type":"WORKSPACE","payment":"FALSE"});
						}
					}),
					new Ext.Button({
						...
						handler:function() {
							...
							if (isMsg == true) {
								Ext.Msg.show({title:"안내",msg:"본사발주품목은 삭제할 수 없습니다.<br />해당 항목을 제외하고 삭제하시겠습니까?",buttons:Ext.Msg.OKCANCEL,icon:Ext.MessageBox.QUESTION,fn:function(button) {
									if (button == "ok") {
										var checked = Ext.getCmp("PaymentList").selModel.getSelections();
										for (var i=0, loop=checked.length;i<loop;i++) {
											if (checked[i].get("type") == "ITEMORDER") {
												Ext.getCmp("PaymentList").selModel.deselectRow(checked[i].get("row"));
											}
										}
										...
										if (idxs.length > 0) {
											Ext.Ajax.request({
												...
												headers:{},
												params:{"action":"payment","do":"item","wno":"<?php echo $this->wno; ?>","mode":"delete","idx":idxs.join(",")}
											});
										} else {
											Ext.Msg.show({title:"안내",msg:"성공적으로 삭제되었습니다.",buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO});
										}
					...
					new Ext.Button({
						...
						handler:function() {
							...
							Ext.Ajax.request({
								...
								headers:{},
								params:{"action":"payment","do":"item","wno":"<?php echo $this->wno; ?>","mode":"modify","data":data}
							});
						}
					})
				],
				...
				store:new Ext.data.GroupingStore({
					...
					reader:new Ext.data.JsonReader({
						root:"lists",
						totalProperty:"totalCount",
						fields:["idx","is_new","group","date","type","gno","tno","workgroup","worktype","itemcode","code","subcode","title","size","unit",{name:"ea",type:"float"},"order_ea",{name:"cost",type:"int"},{name:"price",type:"int"},"payment","cooperation","avgcost","etc"]
					}),
					remoteSort:false,
					groupField:"group",
					sortInfo:{field:"date",direction:"ASC"},
					baseParams:{"wno":"<?php echo $this->wno; ?>","action":"payment","get":"item","date":"<?php echo Request('iErpMonth','cookie') != null ? Request('iErpMonth','cookie') : GetTime('Y-m'); ?>"}
				}),
				trackMouseOver:true,
