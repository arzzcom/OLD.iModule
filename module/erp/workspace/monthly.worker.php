			...
			new Ext.grid.EditorGridPanel({
						...
						handler:function() {
							...
							Ext.Ajax.request({
								...
								success:function() {
									...
									Ext.getCmp("MonthlyList").getStore().commitChanges();
								},
								failure:function() {
									Ext.Msg.show({title:"안내",msg:"서버에 이상이 있어 저장하지 못하였습니다.<br />잠시후 다시 시도해보시기 바랍니다.",buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO});
								},
								headers:{},
								params:{"action":"monthly","do":"worker","mode":"save","wno":"<?php echo $this->wno; ?>","date":Ext.getCmp("month").getValue(),"data":data}
							});
						}
					})
				],
				...
				store:new Ext.data.GroupingStore({
					...
					reader:new Ext.data.JsonReader({
						root:"lists",
						totalProperty:"totalCount",
						fields:["idx","type","cno","repto","cooperation",{name:"prev",type:"int"},{name:"prev_tax",type:"int"},{name:"prev_total",type:"int"},{name:"monthly",type:"int"},{name:"monthly_tax",type:"int"},{name:"monthly_total",type:"int"},{name:"total",type:"int"},{name:"total_tax",type:"int"},{name:"total_total",type:"int"},{name:"remain",type:"int"}]
					}),
					remoteSort:false,
					groupField:"type",
					sortInfo:{field:"idx",direction:"ASC"},
					baseParams:{"wno":"<?php echo $this->wno; ?>","action":"monthly","get":"worker","date":"<?php echo Request('iErpMonth','cookie') != null ? Request('iErpMonth','cookie') : GetTime('Y-m'); ?>"}
				}),
				autoScroll:true,
				trackMouseOver:true,
