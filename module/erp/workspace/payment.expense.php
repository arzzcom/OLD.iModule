		...
		items:[
			new Ext.grid.EditorGridPanel({
						...
						handler:function() {
							...
							if (idxs.length > 0) {
								Ext.Ajax.request({
									...
									headers:{},
									params:{"action":"payment","do":"expense","wno":"<?php echo $this->wno; ?>","mode":"delete","idx":idxs.join(",")}
								});
							} else {
								Ext.Msg.show({title:"안내",msg:"성공적으로 삭제되었습니다.",buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO});
							}
					...
					new Ext.Button({
						...
						handler:function() {
							...
							Ext.Ajax.request({
								...
								headers:{},
								params:{"action":"payment","do":"expense","wno":"<?php echo $this->wno; ?>","mode":"modify","data":data}
							});
						}
					})
				],
				...
				store:new Ext.data.GroupingStore({
					...
					groupField:"group",
					sortInfo:{field:"date",direction:"ASC"},
					baseParams:{"wno":"<?php echo $this->wno; ?>","action":"payment","get":"expense","date":"<?php echo Request('iErpMonth','cookie') != null ? Request('iErpMonth','cookie') : GetTime('Y-m'); ?>"}
				}),
				trackMouseOver:true,
