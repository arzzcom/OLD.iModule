...
ContentArea = function(viewport) {
	...
	var OrderStore = new Ext.data.Store({
		...
		remoteSort:false,
		sortInfo:{field:"date",direction:"ASC"},
		baseParams:{"wno":"<?php echo $this->wno; ?>","action":"outsourcing","get":"order","mode":"list","date":"<?php echo Request('iErpMonth','cookie') != null ? Request('iErpMonth','cookie') : GetTime('Y-m'); ?>"}
	});
				...
				new Ext.Button({
					...
					handler:function() {
						Ext.Msg.show({title:"안내",msg:"기존의 첨부파일이 있다면, 현재의 파일로 대체됩니다.<br />계속 진행하시겠습니까?",buttons:Ext.Msg.OKCANCEL,icon:Ext.Msg.QUESTION,fn:function(button) {
							if (button == "ok") {
								Ext.getCmp("FileForm").getForm().submit({url:"<?php echo $this->moduleDir; ?>/exec/Workspace.do.php?wno=<?php echo $this->wno; ?>&action=outsourcing&do=file&mode=upload&step="+step+"&idx="+idx,waitMsg:"파일을 업로드중입니다."});
							}
						}});
					}
				}),
		...
		new Ext.Window({
			...
			items:[
				new Ext.grid.EditorGridPanel({
					...
					store:new Ext.data.GroupingStore({
						...
						baseParams:{"wno":"<?php echo $this->wno; ?>","action":"outsourcing","get":"order","mode":"item","idx":idx}
					}),
			...
			buttons:[
				new Ext.Button({
					...
					handler:function() {
						ExcelConvert("<?php echo $this->moduleDir; ?>/exec/GetExcel.do.php?action=workspace&get=order&idx="+idx);
					}
				}),
				new Ext.Button({
					text:"수정",
			...
			listeners:{show:{fn:function() {
				Ext.getCmp("OrderForm").getForm().load({url:"<?php echo $this->moduleDir; ?>/exec/Workspace.get.php?wno=<?php echo $this->wno; ?>&action=outsourcing&get=order&mode=data&idx="+idx,waitMsg:"데이터를 로딩중입니다."});
			}}}
		}).show();
	}
			...
			buttons:[
				new Ext.Button({
					...
					handler:function() {
						...
						Ext.getCmp("OrderForm").getForm().findField("data").setValue(GetGridData(Ext.getCmp("OrderListPanel")));
						Ext.getCmp("OrderForm").getForm().submit({url:"<?php echo $this->moduleDir; ?>/exec/Workspace.do.php?action=outsourcing&do=add&wno=<?php echo $this->wno; ?>",waitMsg:"데이터를 전송중입니다.",submitEmptyText:false});
					}
				}),
				new Ext.Button({
					text:"취소",
								...
								Ext.Msg.show({title:"안내",msg:"발주요청서명을 정말 삭제하시겠습니까?",buttons:Ext.Msg.OKCANCEL,icon:Ext.Msg.QUESTION,fn:function(button) {
									if (button == "ok") {
										Ext.Ajax.request({
											...
											headers:{},
											params:{"action":"outsourcing","do":"delete","wno":<?php echo $this->wno; ?>,"idx":data.get("idx")}
										});
									}
								}});
